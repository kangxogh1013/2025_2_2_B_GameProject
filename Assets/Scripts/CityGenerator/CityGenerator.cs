...
public class CityGenerator : MonoBehaviour
{
    ...
    {
        return (x % roadlnterval == 0) || (z % roadlnterval == 0);
    }

    void CreateRoads()                              //길 생성 함수
    {
    ...
    void CreateBuildings()
    {
        for (int x = 0; x < cityWidth; x++)
        {
            for (int z = 0; z < cityHeight; z++)
            {
                if (!IsRoad(x, z))                          //추후 랜덤으로 타워,광장, 공원 등등 추가
                {
                    ...
                    else if (rand < towerChange + plazaChange + parkChance)
                    {
                        CreatePark(x, z);
                    }
                    else
    ...
    void CreatBuilding(int x, int z)
    {
        int floorCount = Random.Range(minBuildingHeight, maxBuildingHeight);
        float height = floorCount * floorHeight;
    ...
    {
        for (int x = 0; x < cityWidth; x += roadlnterval)
        {
            for (int z = 0; z < cityHeight; z += roadlnterval)
            {
